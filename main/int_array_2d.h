...
void loadfromfile2d(FILE* in) { 
	...
	for (; getline(line, MAX_X, in) > 0 && nextfreeY_2d < MAX_Y - 1; nextfreeY_2d++)
		//оставл€ет как минимум одну строку "в запасе" (нужно дл€ буфера в main.c)
...
int alphabetRUY(int y1, int y2) //works with ASCII
{
	...
	do {
		...
		* при этом символ '\0' ставим в самый низкий приоритет
		  дл€ того, чтобы строки без русских букв уходили на дно*/
		if (st1 == 3)
			c1 = array_2d_r(i, y1) - ('а' & 255); 
			/* побитовое » использовал изза шизы в кодировке
			*  компил€тор у мен€ расшир€ет 8бит в 16/32бита 
			*  методом распространени€ знака, а из файла код,
			*  полученный методом вставки нулей слева
			*  те компил€тор думает что это -32, а файл дает 224 */
		else if (st1 == 4)
			c1 = array_2d_r(i, y1) - ('ј' & 255);
		else if (st1 == -1)
			c1 = ('€' & 255) + 1;
